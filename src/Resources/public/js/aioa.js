...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const positionField = document.querySelector(
    "#sub_aioa_enable .position-field"
  );
...
window.addEventListener("DOMContentLoaded", function () {
  let aioa_icontype = document.querySelector(
...
function updateWidgetSize(value) {
  const widgetSizeField = document.querySelector(
    "input[name='aioa_widget_size'][value='Oversize']"
  );
  const regularSizeField = document.querySelector(
    "input[name='aioa_widget_size'][value='Regular Size']"
  );
...
document.addEventListener("DOMContentLoaded", function () {
  function toggleFields() {
    ...
    var customPositionFields = document.querySelectorAll(
...
function ChangeIcon(val) {
  ...
  arrSize.forEach(function (item) {
    item.setAttribute(
      "src",
      "bundles/skynettechnologiescontaoallinoneaccessibility/icons/" +
        val +
        ".svg"
    );
  });
...
function saveData() {
  ...
  const customPositionX =
    document.getElementById("ctrl_aioa_custom_position_lr")?.value || 0; // X position value
  const customPositionY =
    document.getElementById("ctrl_aioa_custom_position_bt")?.value || 0; // Y position value
  const xPositionDirection = document.getElementById(
    "ctrl_aioa_custom_position_rl"
  )?.value; // Left/Right select
  const yPositionDirection = document.getElementById(
    "ctrl_aioa_custom_position_tb"
  )?.value; // Top/Bottom select
  ...
  var is_widget_custom_position = document.getElementById(
    "opt_aioa_position_enable_0"
  ).checked
    ? 1
    : 0;
  var is_widget_custom_size = document.getElementById(
    "opt_aioa_customsize_enable_0"
  ).checked
    ? 1
    : 0;
  //  var widget_icon_size_custom = document.getElementById("ctrl_aioa_custom_size")?.value || "";
  var widget_icon_size_custom =
    document.querySelector(".custom-size-field input")?.value || "";

  if (is_widget_custom_position == 1 && customPositionX > 250) {
      alert("Please enter valid icon position.!");
      return;
  }

  if (is_widget_custom_position == 1 && customPositionY > 250) {
      alert("Please enter valid icon position.!");
      return;
  }
  ...
  if (
    ...
    alert("Please enter valid icon size.!");
    return;
  }

  var icon_type =
    document.querySelector('input[name="aioa_icon_type"]:checked')?.value || "";
  var icon_size =
    document.querySelector('input[name="aioa_icon_size"]:checked')?.value || "";
  var selectedValue = document.querySelector(
    "input[name='aioa_widget_size']:checked"
  )?.value;
